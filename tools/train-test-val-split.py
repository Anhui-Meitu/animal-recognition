...
def split_files(images, labels, train_ratio=0.8, val_ratio=0.1):
    ...
    return (images[:train_count], images[train_count:train_count + val_count], images[train_count + val_count:], labels[:train_count], labels[train_count:train_count + val_count], labels[train_count + val_count:])
...
if __name__ == "__main__":
    ...
    images = glob(os.path.join(directory, '*.jpg')) + glob(os.path.join(directory, '*.png')) + glob(os.path.join(directory, '*.jpeg'))
    ...
    # split the files
    train_files, val_files, test_files, train_labels, val_labels, test_labels = split_files(images, labels)
