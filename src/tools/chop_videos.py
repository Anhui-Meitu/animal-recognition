    ...
    for video_file in glob(os.path.join(video_files_dir, '*.mp4')):
        try:
            ...
            while True:
                ...
                if frame_count % (cap.get(cv2.CAP_PROP_FPS) * interval) == 0:
                    image_file = os.path.join(output_dir, f"{os.path.basename(video_file)}_frame_{frame_count}.jpg")
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--video_files_dir", type=str, required=True, help="Directory containing video files")
    arg_parser.add_argument("--output_dir", type=str, required=True, help="Directory to save images")
    arg_parser.add_argument("--interval", type=int, default=1, help="Interval in seconds to save images")
    ...
    chop_videos_into_images(args.video_files_dir, args.output_dir, args.interval)